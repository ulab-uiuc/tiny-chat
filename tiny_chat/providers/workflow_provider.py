...
class RequestHandler:
    ...
    async def process(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
...
class WorkflowProvider(BaseModelProvider):
    ...
    def register_request_handler(
        cls, name: str, handler_class: type[RequestHandler]
    ) -> None:
    ...
    def register_response_parser(
        cls, name: str, parser_class: type[ResponseParser]
    ) -> None:
    ...
    def is_tinychat_mode(self) -> bool:
        ...
        return (
            custom_config.get("use_tinychat_agenerate", True) if custom_config else True
        )
    ...
    def _get_agenerate_model_name(self) -> str:
        ...
        if custom_config:
            ...
            fallback = custom_config.get("fallback_model")
            if fallback:
                return str(fallback)
    ...
    async def process(self, prompt: str, **kwargs: Any) -> dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                str(endpoint), json=request_data, headers=headers, timeout=30.0
            )
